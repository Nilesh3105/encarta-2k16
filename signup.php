...
if(isset($_POST['user_signup'])) {
	...
	{
		if(!preg_match("/^[A-Za-z ]{2,60}$/",$_POST['name'])) {
			alert("<center><strong>Failure!</strong> Invalid characters used in name.</center>", "danger");
			$flag = false;
		}
		if(!preg_match("/^[A-Za-z ]{2,100}$/",$_POST['college'])) {
			alert("<center><strong>Failure!</strong> Invalid characters used in college.</center>", "danger");
			$flag = false;
		}
		if(!preg_match("/^[A-Za-z ]{2,}$/",$_POST['branch'])) {
			alert("<center><strong>Failure!</strong> Invalid branch.</center>", "danger");
			$flag = false;
		}
		if(!is_numeric($_POST['sem'])) {
			alert("<center><strong>Failure!</strong> Invalid Semester.</center>", "danger");
			$flag = false;
		}
		...
		if(!preg_match("/^[A-Za-z0-9!@#$%^&*()_]{1,60}$/",$_POST['password'])) {
					...
					<form action="<?php $_SERVER['PHP_SELF']; ?>" method="POST" style="margin-top: 12px;">
						...
						<div class="form-group label-floating">
							<div class="input-group">
								...
								<label for="sem" class="control-label">Current Semester</label>
